...
export default function AdminStudySpaces() {
  ...
  const [newSpace, setNewSpace] = useState({ name: "", location: "", totalSeats: "" });
  const [savingSpace, setSavingSpace] = useState(false);

  const [newTable, setNewTable] = useState({ code: "", type: "SINGLE", capacity: "" });
  ...
  const submitSpace = async (e) => {
    ...
    if (!newSpace.name.trim() || !newSpace.location.trim() || !Number.isFinite(total) || total < 0) {
      toast.error("Enter name, location, and total seats (0 or more).");
      return;
    }
    setSavingSpace(true);
    try {
    ...
    <div className="min-h-screen bg-slate-50 font-poppins flex flex-col">
      ...
      <div className="flex-1 flex flex-col min-h-0 max-w-7xl mx-auto w-full px-6 pt-4 pb-6">
        <div className="mb-4 shrink-0">
          <p className="text-[10px] font-bold text-violet-600 uppercase tracking-widest">Study spaces</p>
          <h1 className="text-lg font-extrabold text-slate-900 leading-tight">Manage spaces & tables</h1>
          <p className="text-xs text-slate-400 mt-0.5">Add spaces, tables, and seat counts for student bookings.</p>
        </div>

        <div className="flex-1 flex gap-5 min-h-0">
        <div className="w-[380px] shrink-0 flex flex-col gap-4 min-h-0">
          <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm">
            <h2 className="text-sm font-extrabold text-slate-800 mb-3">New study space</h2>
            <form onSubmit={submitSpace} className="space-y-3">
              ...
              <input
                ...
                onChange={(e) => setNewSpace((s) => ({ ...s, location: e.target.value }))}
                className="w-full bg-slate-50 border border-slate-200 text-sm px-3 py-2 rounded-xl outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100"
              />
              <input
                type="number"
                ...
                onChange={(e) => setNewSpace((s) => ({ ...s, totalSeats: e.target.value }))}
                className="w-full bg-slate-50 border border-slate-200 text-sm px-3 py-2 rounded-xl outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100"
              />
              <p className="text-[10px] text-slate-400 leading-snug">
                Each table you add increases the space total seats automatically.
              </p>
              <button
                type="submit"
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-0.5 min-h-0">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Spaces</p>
            {loading ? (
              ...
              spaces.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5 line-clamp-2">{s.location}</p>
                    <div className="flex flex-wrap gap-2 mt-2 text-[10px] font-semibold text-slate-600">
                      ...
                      <span className="bg-violet-50 text-violet-700 px-2 py-0.5 rounded-md">
                        {s.groupTables ?? 0} group · {s.singleTables ?? 0} single
                      </span>
                    </div>
                  </button>
                );
        ...
        <div className="flex-1 flex flex-col bg-white rounded-2xl border border-slate-100 shadow-md overflow-hidden min-h-0">
          {!selectedId ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-8">
              ...
              <p className="font-bold text-slate-700 text-lg">Select a study space</p>
              <p className="text-sm text-slate-400 max-w-sm">
                Choose a space on the left to view tables and add seats by table.
              </p>
            </div>
          ) : detailLoading ? (
            ...
            <>
              <div className="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-violet-50 to-white flex items-start gap-4">
                ...
                <div className="min-w-0">
                  <p className="font-bold text-slate-900">{detail.space.name}</p>
                  <p className="text-xs text-slate-500 mt-0.5">{detail.space.location}</p>
                  <p className="text-[11px] text-slate-600 mt-2">
                    Status: <strong>{detail.space.status}</strong>
                    {" · "}
                    Available <strong>{detail.space.availableSeats}</strong> of <strong>{detail.space.totalSeats}</strong> seats
                  </p>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
                  <h3 className="text-xs font-extrabold text-slate-700 uppercase tracking-wide mb-3">Add table</h3>
                  <form onSubmit={submitTable} className="flex flex-wrap gap-2 items-end">
                    <div className="flex-1 min-w-[120px]">
                      <label className="text-[10px] font-semibold text-slate-500 block mb-1">Code</label>
                      <input
                        type="text"
                        value={newTable.code}
                        onChange={(e) => setNewTable((t) => ({ ...t, code: e.target.value }))}
                        placeholder="e.g. T1"
                    ...
                    <div className="w-[140px]">
                      ...
                      <select
                        value={newTable.type}
                        onChange={(e) => setNewTable((t) => ({ ...t, type: e.target.value }))}
                        className="w-full bg-white border border-slate-200 text-sm px-3 py-2 rounded-lg outline-none focus:border-indigo-300"
                      >
                    ...
                    <div className="w-[100px]">
                      <label className="text-[10px] font-semibold text-slate-500 block mb-1">Seats</label>
                      <input
                        type="number"
                        min={1}
                ...
                <div>
                  <h3 className="text-xs font-extrabold text-slate-700 uppercase tracking-wide mb-2">Tables</h3>
                  {!detail.tables?.length ? (
                    ...
                    <div className="space-y-2">
                      {detail.tables.map((tbl) => (
                        ...
                        >
                          <div>
                            <span className="font-bold text-slate-800">{tbl.code}</span>
                            <span className="text-slate-400 mx-2">·</span>
                            <span className="text-slate-600">{tbl.type === "GROUP" ? "Group" : "Single"}</span>
                          </div>
                          <div className="text-xs font-semibold text-slate-600">
                            {tbl.availableSeats}/{tbl.capacity} seats available
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="flex-1 flex items-center justify-center text-slate-400 text-sm">Could not load space.</div>
          )}
        </div>
        </div>
      </div>
