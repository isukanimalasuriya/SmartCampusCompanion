...
const OfferSkillModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/skills/offer", {
        method: "POST",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[90vh] flex flex-col">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
          <h2 className="text-xl font-bold text-gray-900">Offer a Skill</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                required
                className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 outline-none transition text-sm cursor-pointer"
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="relative group">
                <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-indigo-400 group-focus-within:text-indigo-600 transition-colors" size={16} />
                <input
                  required
                  type="date"
                  className="w-full pl-10 pr-3 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 outline-none transition-all text-xs font-semibold text-gray-700 appearance-none shadow-sm"
              ...
              <div className="relative group">
                <Clock className="absolute left-3 top-1/2 -translate-y-1/2 text-indigo-400 group-focus-within:text-indigo-600 transition-colors" size={16} />
                <input
                  required
                  type="time"
                  className="w-full pl-10 pr-3 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 outline-none transition-all text-xs font-semibold text-gray-700 appearance-none shadow-sm"
          ...
          <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 flex items-center justify-between">
            ...
            <div className="flex-1 text-right">
              ...
              <span className="text-xs font-mono text-gray-500">{user.id.slice(-5)}</span>
            </div>
          </div>
